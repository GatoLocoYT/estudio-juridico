...
const VALID_STATUS = ["open", "pending", "closed", "archived"];
...
router.get("/", requireAdmin, (req, res) => {
  ...
  const { safeSort, safeDir } = pickSort(
    req.query,
    ["created_at", "updated_at", "opened_at", "status", "priority"],
    "created_at"
  );
  ...
  const total =
    db()
      .prepare(`SELECT COUNT(*) as c FROM cases c ${whereSql}`)
      .get(params)?.c ?? 0;
...
// ✅ Acción rápida: cambiar SOLO estado (dashboard)
router.put("/:id/status", requireAdmin, (req, res) => {
  ...
  const current = db()
    .prepare(`SELECT id, status, closed_at FROM cases WHERE id = ? AND deleted_at IS NULL`)
    .get(id);

  if (!mustExistOr404(res, current, "case")) return;

  // closed_at: set si cerrás, clear si reabrís
  const nextClosedAt = status === "closed"
    ? (current.closed_at || new Date().toISOString().slice(0, 10)) // YYYY-MM-DD
    : null;
  ...
  db()
    ...
    .run(status, nextClosedAt, id);

  return res.json({ ok: true });
});

// ✅ Acción rápida: cambiar SOLO prioridad (dashboard)
router.put("/:id/priority", requireAdmin, (req, res) => {
  const id = Number(req.params.id);
  if (!Number.isInteger(id)) return apiError(res, 400, "VALIDATION_ERROR", "Invalid id");

  const priority = trimOrNull(req.body.priority);
  if (!priority || !VALID_PRIORITY.includes(priority)) {
    return apiError(res, 400, "VALIDATION_ERROR", "Invalid priority", { field: "priority" });
  }
...
router.post("/", requireAdmin, (req, res) => {
  ...
  const opened_at = trimOrNull(req.body.opened_at) ?? null; // YYYY-MM-DD
  ...
  if (!VALID_STATUS.includes(status)) return apiError(res, 400, "VALIDATION_ERROR", "Invalid status", { field: "status" });
  if (!VALID_PRIORITY.includes(priority)) return apiError(res, 400, "VALIDATION_ERROR", "Invalid priority", { field: "priority" });
...
router.delete("/:id", requireAdmin, (req, res) => {
  ...
  const info = db()
    .prepare(`UPDATE cases SET deleted_at = datetime('now'), updated_at = datetime('now') WHERE id = ? AND deleted_at IS NULL`)
    .run(id);
